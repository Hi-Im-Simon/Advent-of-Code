...
def init():
    if len(sys.argv) == 2:
        ...
        if str(year) in os.listdir(PATH):
            day = (
                max(
                    [
                        int(f_name.strip("day-").strip(".py"))
                        for f_name in os.listdir(f"{PATH}/{year}")
                        if "day-" in f_name
                    ]
                )
                + 1
    ...
    if str(year) in os.listdir(PATH) and file_name_py in os.listdir(f"{PATH}/{year}"):
        if not (
            input("This file already exists. Overwrite with base data? (Y/n) ").upper()
            == "Y"
        ):
...
class Day{ day }:
    def __init__(self, use_example_input: bool = False) -> None:
        with open(
            f"{{__file__}}/../{{('example-' if use_example_input else '')}}inputs/input-{file_number}.txt"
        ) as file:
